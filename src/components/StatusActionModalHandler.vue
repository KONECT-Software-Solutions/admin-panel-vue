<template>
  <Status0Modal
    v-if="meetingData.status === '0' && keepStatusModalOpen"
    @close="$emit('close')"
    :meetingData="meetingData"
    :formattedDate="formattedDate"
    @accept-meeting="$emit('accept-meeting')"
    @reject-meeting="handleRejectMeeting" />
  <Status1Modal
    v-if="meetingData.status === '1' && keepStatusModalOpen"
    @close="$emit('close')"
    :meetingData="meetingData"
    :formattedDate="formattedDate" />
  <Status2Modal
    v-if="meetingData.status === '2' && keepStatusModalOpen"
    @close="$emit('close')"
    @save-result="handleSaveResult"
    :meetingData="meetingData" />
  <Status3Modal
    v-if="meetingData.status === '3' && keepStatusModalOpen"
    @close="$emit('close')"
    :rejectReason="meetingData.cancel_reason" />
  <Status6Modal
    v-if="meetingData.status === '6' && keepStatusModalOpen"
    @close="$emit('close')"
    :cancelReason="meetingData.cancel_reason" />
</template>

<script setup>
import Status0Modal from "../components/Status0Modal.vue";
import Status1Modal from "../components/Status1Modal.vue";
import Status2Modal from "../components/Status2Modal.vue";
import Status3Modal from "../components/Status3Modal.vue";
import { computed } from "vue";
import { formatDate } from "../utils";
import Status6Modal from "../components/Status6Modal.vue";

const emits = defineEmits(["close", "accept-meeting", "reject-meeting", "save-result"]);
...
const handleSaveResult = (meetingResult, meetingResultText) => {
  emits("save-result", meetingResult, meetingResultText);
};
