  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 shadow-lg">
      ...
      <div class="mt-2 flex flex-wrap">
        <span v-for="(count, status) in meetingStatusSummary" :key="status" :class="statusColors[status]"
    ...
    <div v-if="showTopAttorneys" class="bg-white border border-gray-200 rounded-lg p-6 shadow-lg">
      <div class="flex justify-between items-center mb-4 bg-gray-800 p-2 rounded-t-lg">
        <div class="flex items-center p-2">
          <i class="ri-award-line text-2xl text-white"></i>
          <div class="ml-2 text-lg font-bold text-white">En çok Görüşme Yapan Avukatlar</div>
        </div>
      </div>
      <ul class="mt-2 space-y-2">
        <li v-for="(attorney, index) in topAttorneys" :key="index"
          class="flex items-center justify-between text-lg font-medium text-black bg-gray-100 h-10 rounded-lg">
          <div class="flex items-center">
            <div class="bg-gray-800 text-white rounded-full h-8 w-8 flex items-center justify-center font-bold">
              {{ index + 1 }}
            </div>
          ...
          <div class="text-xl border-l-2  border-gray-800 px-4">{{ attorney.meetings }}</div>
        </li>
      </ul>
    </div>
