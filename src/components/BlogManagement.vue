<template>
  <div>
    ...
    <shadow-box class="p-6">
      ...
      <table v-if="blogsData?.length" class="w-full mt-4 table-fixed">
        <thead>
          <tr>
            <th
...
const totalBlogView = computed(() => {
  let totalView = 0;
  props.blogsData.forEach((blog) => {
    totalView += blog.view_count || 0;
  });
...
const openURL = (slug, id) => {
  ...
  window.open(`https://tiryakihukuk.com/blog/${slug}/${id}`, "_blank");
};
