<template>
  ...
  <div
    ...
    <div class="bg-white rounded-lg p-4 max-w-sm w-full">
      <div class="mt-1">
        <Loading v-if="modalState.showLoadingModal" text="Blog Yazısı Güncelleniyor..." />
      </div>
    </div>
  </div>
    ...
    <div class="bg-white rounded-lg p-4 max-w-sm w-full">
      ...
      <div class="flex justify-end">
        <button
          ...
          class="bg-red-600 text-white px-4 py-1 font-medium rounded mb-2">
          Tekrar Dene
        </button>
      </div>
    </div>
    ...
    <div class="relative p-4 w-full max-w-4xl max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow">
        ...
        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
          <h3 class="text-lg font-semibold text-gray-900">Blog Yazısını Güncelle</h3>
...
const handleSubmit = async (blogDataToUpdate) => {
  ...
  try {
    ...
    modalState.errorMessage = 'Blog güncellenirken bir hata oluştu. Lütfen tekrar deneyin.';  // Error message
  }
};
