<template>
  ...
  <div v-if="showLoadingModal" class=" font-medium text-sm fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
        <div class="bg-white rounded-lg p-4 max-w-sm w-full">
            <div class="mt-1">
                  <Loading
                    v-if="showLoading"
                    text="Blog Yazısı Yükleniyor..."
                />
      ...
      <form @submit.prevent="handleSubmit" class="p-4 md:p-5">
          <div class="grid gap-4 mb-4 grid-cols-2">
              ...
              <div class="col-span-2 sm:col-span-1">
                  <label for="file-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fotoğraf Seç</label>
                  <input
                      type="file"
                      id="file_input"
                      ref="fileInput"
          ...
          <button type="submit" :disabled="showLoading" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-600">  
            Blog Yazısı Ekle
          </button>
      </form>
  </div>
